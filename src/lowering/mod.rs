...
impl<'a, 'ir> Lowering<'a, 'ir> {
    ...
    fn lower_expr(&mut self, expr: &ast::Expr) -> ir::Expr<'ir> {
        match &expr.kind {
            ...
            ast::ExprKind::Lit(lit) => {
                ...
                let ty = match lit {
                    ir::ExprLit::Bool(_) => self.ctx.types.bool,
                    ir::ExprLit::String(_) => {
                        self.ctx.allocator.alloc(ir::Ty::Ptr(self.ctx.types.u8))
                    }
                    ir::ExprLit::Null => self.ctx.types.null,
                    _ => self.ctx.allocator.alloc(ir::Ty::Infer(None)),
                };
    ...
    fn lower_stmt(&mut self, stmt: &ast::Stmt) -> ir::Stmt<'ir> {
        match stmt {
            ast::Stmt::Local(ast::Variable { ty, name, value }) => ir::Stmt {
                id: self.next_id(),
...
pub fn lower<'ir>(ctx: &mut Context<'ir>, items: Vec<ast::Item>) -> Package<'ir> {
    ...
    for item in items {
        if let Some(item) = lowering.lower_item(&item) {
            lowering.ir.root_items.push(item);
        }
    }
