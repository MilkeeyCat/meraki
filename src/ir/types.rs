...
impl Ty<'_> {
    pub fn size(&self, bitness: usize) -> usize {
        match self {
            Self::Void => 0,
            ...
            Self::Int(int) => match int {
                ...
                IntTy::I32 => 4,
                IntTy::I64 => 8,
                IntTy::Isize => bitness / 8,
            },
            Self::UInt(uint) => match uint {
                UintTy::U8 => 1,
                UintTy::U16 => 2,
                UintTy::U32 => 4,
                UintTy::U64 => 8,
                UintTy::Usize => bitness / 8,
            },
            Self::Ptr(_) | Self::Fn(_, _) => bitness / 8,
            Self::Array(ty) => ty.ty.size(bitness) * ty.len,
            Self::Struct(ty) => ty.iter().map(|(_, ty)| ty.size(bitness)).sum(),
