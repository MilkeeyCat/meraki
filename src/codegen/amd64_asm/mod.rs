...
use super::Codegen;
use crate::{
    ir::{Block, Expr, ExprKind, ExprLit, Id, Item, ItemFn, Stmt, Ty, Variable},
...
use indoc::formatdoc;
use operand::{
    Base, Destination, EffectiveAddress, Immediate, ImmediateStrLitError, Memory, Source,
};
...
impl<'a, 'ir> Amd64Asm<'a, 'ir> {
    ...
    fn expr_dest(&mut self, expr: &Expr) -> Result<Destination, Amd64AsmError> {
        Ok(match expr.kind {
            ExprKind::Ident(id) => self.variables[&id].clone(),
            ExprKind::Unary(op, expr) if op == UnOp::Deref => {
                let ty = self.ctx.resolve_ty(expr.ty);
                let ty_size = self.ty_size(ty).try_into()?;
                let r = self.allocator.alloc(ty_size)?;
    ...
    fn expr(&mut self, expr: &Expr, dest: Option<&Destination>) -> Result<(), Amd64AsmError> {
        Ok(match expr.kind {
            ExprKind::Binary(op, lhs, rhs) => {
                ...
                match op {
                    BinOp::Assign => {
                        let expr_dest = self.expr_dest(lhs)?;

                        self.expr(rhs, Some(&expr_dest))?;

                        if let Some(dest) = dest {
                            self.mov(&expr_dest.into(), dest, signed)?;
                        }
                    }
                    BinOp::Add => {
                        if let Some(dest) = dest {
                            ...
                            self.add(&r_lhs.into(), &r_rhs.into(), dest, signed)?;
                    ...
                    BinOp::Sub => {
                        if let Some(dest) = dest {
                            ...
                            self.sub(&r_lhs.into(), &r_rhs.into(), dest, signed)?;
                    ...
                    BinOp::Mul => {
                        if let Some(dest) = dest {
                            ...
                            self.mul(&r_lhs.into(), &r_rhs.into(), dest, signed)?;
                    ...
                    BinOp::Div => {
                        if let Some(dest) = dest {
                            ...
                            self.div(&r_lhs.into(), &r_rhs.into(), dest, signed)?;
                    ...
                    BinOp::LogicalOr => {
                        if let Some(dest) = dest {
                            self.logical_or(expr, dest, None)?
                        }
                    }
                    BinOp::LogicalAnd => {
                        if let Some(dest) = dest {
                            self.logical_and(expr, dest, None)?
                        }
                    }
            ...
            ExprKind::Unary(op, inner_expr) => {
                if let Some(dest) = dest {
                    let signed = self.ctx.resolve_ty(inner_expr.ty).signed();

                    match op {
                        UnOp::LogicalNot => {
                            self.expr(inner_expr, Some(dest))?;
                        ...
                        UnOp::Address => {
                            let expr_dest = self.expr_dest(inner_expr)?;
                            let r = self.allocator.alloc(OperandSize::Qword)?;

                            self.lea(&r.into(), &expr_dest.into());
                            self.mov(&r.into(), dest, signed)?;

                            self.allocator.free(r)?;
                        }
                        UnOp::Deref => {
                            let expr_dest = self.expr_dest(expr)?;
            ...
            ExprKind::Lit(lit) => {
                if let Some(dest) = dest {
                    if let ExprLit::String(str) = lit {
                        ...
                        self.mov(&Source::Immediate(label.into()), dest, false)?;
                    } else {
                        ...
                        self.mov(&Source::Immediate(lit.try_into()?), dest, signed)?;
                    }
                }
            }
            ExprKind::Ident(id) => {
    ...
    fn local(&mut self, stmt: &Variable) -> Result<(), Amd64AsmError> {
        let size = self.ty_size(self.ctx.resolve_ty(stmt.ty));
    ...
    fn logical_or(
        ...
        match expr.kind {
            ExprKind::Binary(BinOp::LogicalOr, lhs, rhs) => {
                self.logical_or(lhs, dest, Some((set.clone(), end.clone())))?;
                self.logical_or(rhs, dest, Some((set.clone(), end.clone())))?;
            }
            _ => {
                self.expr(expr, Some(dest))?;
                self.cmp(dest, &Source::Immediate(Immediate::UInt(0)));
    ...
    fn logical_and(
        ...
        match expr.kind {
            ExprKind::Binary(BinOp::LogicalAnd, lhs, rhs) => {
                self.logical_and(lhs, dest, Some((set.clone(), end.clone())))?;
                self.logical_and(rhs, dest, Some((set.clone(), end.clone())))?;
            }
            _ => {
                self.expr(expr, Some(dest))?;
                self.cmp(dest, &Source::Immediate(Immediate::UInt(0)));
    ...
    fn bitwise(
        ...
        self.text.push_str(&format!("\t{op} {rhs}\n"));
