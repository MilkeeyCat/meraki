...
impl<T: Iterator<Item = Result<Token, LexerError>>> Parser<T> {
    ...
    fn stmt(&mut self) -> Result<Stmt, ParserError> {
        match self.cur_token.as_ref().unwrap() {
            ...
            Token::Let => Ok(Stmt::Item(self.var_decl()?)),
            Token::Continue => {
    ...
    fn var_decl(&mut self) -> Result<Item, ParserError> {
...
mod test {
    ...
    use crate::{
        lexer::Lexer,
        parser::{
            BinOp, Expr, ExprBinary, ExprCast, ExprIdent, ExprLit, ExprUnary, IntTy, Item, ItemVar,
            ParserError, Stmt, Ty, UintTy, UnOp,
        },
    };
